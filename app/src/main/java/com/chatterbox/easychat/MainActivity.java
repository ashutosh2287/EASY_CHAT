     ...
     void getFCMToken() {
         FirebaseMessaging.getInstance().getToken().addOnCompleteListener(task -> {
             ...
             if (FirebaseUtil.currentUserDetails() != null) {
                 Objects.requireNonNull(FirebaseUtil.currentUserDetails())
                         .update("fcmToken", token)
                         .addOnSuccessListener(unused -> Log.d("FCM", "Token saved to Firestore"))
                         .addOnFailureListener(e -> Log.w("FCM", "Failed to save token", e));
             } else {
                 Log.w("FCM", "User not signed in; skipping token update");
             }
         });
     }
